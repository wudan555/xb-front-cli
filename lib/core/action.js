...
const { getRepo, URL } = require("../config/common.js");
 ...
 * @param {*} lastArgs 剩余参数
 */
const createProjectAction = async (name, url) => {
  try {
    let pro_url = "";
    if (URL) pro_url = URL;
    if (url) pro_url = url;
    if (!pro_url)
      throw new Error(
        "project url is not found,you should run 'xb create pro_name -u http://***.com '"
      );
    // 1.clone项目：使用download-git-repo库
    await download(getRepo(pro_url), name, { clone: true });
